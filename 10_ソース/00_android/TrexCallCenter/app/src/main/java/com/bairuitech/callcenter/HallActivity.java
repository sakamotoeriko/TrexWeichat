...
public class HallActivity extends Activity implements OnItemClickListener,
        ...
        AnyChatUserInfoEvent ,AnyChatTransDataEvent{
    ...
    protected void startBackServce() {
        Intent intent = new Intent();
        intent.setPackage("com.bairuitech.bgservice");
            ...
            case AnyChatDefine.BRAC_VIDEOCALL_EVENT_START:
                if (dialog != null && dialog.isShowing())
                    ...
                    BussinessCenter.getBussinessCenter().onVideoCallStart(
                            dwUserId, dwFlags, dwParam, userStr);
//                } else if (BussinessCenter.sessionItemExtention == null) {
...
//                }
    ...
    public void OnAnyChatTransFile(int dwUserid, String FileName, String TempFilePath, int dwFileLength, int wParam, int lParam, int dwTaskId) {
        Log.d(TAG,"OnAnyChatTransFile");
    }

    @Override
    public void OnAnyChatTransBuffer(int dwUserid, byte[] lpBuf, int dwLen) {
        Log.d(TAG,"OnAnyChatTransBuffer");
        BussinessCenter.getBussinessCenter().onTransBuffer(dwUserid,lpBuf,dwLen);
    }

    @Override
    public void OnAnyChatTransBufferEx(int dwUserid, byte[] lpBuf, int dwLen, int wparam, int lparam, int taskid) {
        Log.d(TAG,"OnAnyChatTransBufferEx");
    }

    @Override
    public void OnAnyChatSDKFilterData(byte[] lpBuf, int dwLen) {
        Log.d(TAG,"OnAnyChatSDKFilterData");
    }
}
